...
#DEFINE MAXVALORC  012                                                // 009 - valor padrao  Máximo de caracteres por linha de valores numéricos -  Potencial 
...
Local aIndAux	    := {} 
Local aLote         := {}
Local nQuant        := 0.00000 //POTENCIAL -> aqui
Local nHPage        := 0
Local nVPage        := 0
Local nPosV         := 0
Local nPosVOld      := 0
...
If Type("oTransp:_Vol")<>"U"
	If ValType(oTransp:_Vol) == "A"
		...
		For nX := 1 to nLenVol
			nXAux := nX
			// Potencial nvolume conforme posicionamento do pedido de vendas 
		...
		aTransp[11]	:= STR(nVolume) // AllTrim(str(nVolume)) // Potencial 
		// potencial 
		...
		aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U",IIF((SM0->M0_CODIGO)=="01","MIL BRASIL",IIF((SM0->M0_CODIGO)=="03","MIL ENERGIA","")),oTransp:_Vol:_Marca:TEXT) // Potencial aqui
	...
	Else
		if nvolume == 0
		  nvolume := 1
		ENDIF
		aTransp[11] := STR(nVolume) // Potencial //IIf(Type("oTransp:_Vol:_qVol:TEXT")<>"U",oTransp:_Vol:_qVol:TEXT,"")
		aTransp[12] := IIf(Type("oTransp:_Vol:_Esp")=="U","",oTransp:_Vol:_Esp:TEXT)
		// aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U","",NoChar(oTransp:_Vol:_Marca:TEXT,lConverte))
		aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U",IIF((SM0->M0_CODIGO)=="01","MIL BRASIL",IIF((SM0->M0_CODIGO)=="03","MIL ENERGIA","")),oTransp:_Vol:_Marca:TEXT) // Potencial aqui
		aTransp[14] := IIf(Type("oTransp:_Vol:_nVol:TEXT")<>"U",oTransp:_Vol:_nVol:TEXT,"")
    ...
    // Alert("valor de atransp[12]: "+atransp[12])
	...
	If (SF2->(FieldPos("F2_ESPECI1")) <>0 .And. !Empty( SF2->(FieldGet(FieldPos( "F2_ESPECI1" )))  )) .Or.;
		...
		For nE := 1 To Len(aEspecie)
			If !Empty(aEspecie[nE])
				nx ++
				cEsp := aEspecie[1]  // potencial 
			EndIf
		Next
		
		cGuarda := ""
		If nx > 1
			cGuarda :=  aEspecie[1] // "Diversos" // Potencial
			
		Else
			cGuarda := cEsp
		EndIf
		
		// Alert("valor de cguarda:"+cGuarda)
...
nForTo		:=	Len(cStrAux)/30  // Potencial valor padrào 25
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*30)+1),30), oFont12N:oFont )  // potencial 
	nLinCalc+=10
Next nX
...
oDanfe:Say(428,002,aTransp[11],oFont08:oFont) // potencial
...
Local cLogo      	:= FisxLogo("1")
Local cChaveCont 	:= ""
Local lConverte     := .T. // GetNewPar("MV_CONVERT",.F.)
Local lMv_Logod := If(GetNewPar("MV_LOGOD", "N" ) == "S", .T., .F.   )
...
oDanfe:Say(215,504,"DATA DE EMISSAO",oFont08N:oFont)
