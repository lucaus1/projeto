	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				EndDo
				*/    
				// customizado mil Potencial
				       //   1              2               3           4 NVOL           5 QVOL             6                7
				aadd(aEspVol,{ cEspecie, SF2->F2_ESPECI1, SF2->F2_ESPECI2, SF2->F2_VOLUME2, SF2->F2_VOLUME1, SF2->F2_PLIQUI , SF2->F2_PBRUTO})
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			*/
			// 
			// customizado mil   - Potencial 
			// VETOR ESPECIE E VOLUME PARA NOTA DE ENTRADA  
			//                   1                 2              3                  4          5                   6                7
			aadd(aEspVol,{SF1->F1_ESPECI1,SF1->F1_ESPECI2,SF1->F1_VOLUME1,SF1->F1_VOLUME2,SF1->F1_VOLUME3,SF1->F1_PLIQUI,SF1->F1_PBRUTO}) 
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					IIF(!((cAliasSD1)->D1_TIPO $"IP" .Or. ((cAliasSD1)->D1_TIPO $ "D" .And. cTpOrig == "P")) ,(cAliasSD1)->D1_TOTAL,0),;
...
If !Empty(aNota)
	...
	nA := 0
    Alert("Conteudo de cMensFis :"+cMensFis)
    Alert("Conteudo de cMensCli :"+cMensCli)
...
For nX := 1 To Len(aVol)		
 // Alert("Entrou na Funcao para dados de aVol")    
 //ALERT("Conteudo para nVol: "+ConvType(aVol[nX][04],8,3)) 
 // potencial mil 
	cString += '<vol>'         
		cString += NfeTag('<qVol>',ConvType(aVol[nX][05]))
		cString += NfeTag('<esp>' ,ConvType(aVol[nX][02],15,0))
		cString += NfeTag('<marca>',ConvType(aVol[nX][03]))
		cString += NfeTag('<nVol>',ConvType(aVol[nX][04],8,3))
		cString += NfeTag('<pesoL>' ,ConvType(aVol[nX][06],15,3))
		cString += NfeTag('<pesoB>' ,ConvType(aVol[nX][07],15,3))
