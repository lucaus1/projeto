	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			If (FieldPos("F1_ESPECI"+cScan))>0
				While ( !Empty(cScan) )
					...
					If !Empty(cEspecie)
						...
						Else
							aadd(aEspVol,{ cEspecie, FieldGet(FieldPos("F1_VOLUME"+cScan)) , 0 , 0})
						EndIf
						// padrao 
		   ...
		   cScan := "1"
				cEspecie := Upper(FieldGet(FieldPos("F1_ESPECI"+cScan)))
				// aadd(aEspVol,{ cEspecie, SF2->F2_ESPECI1, SF2->F2_ESPECI2, SF2->F2_VOLUME2, SF2->F2_VOLUME1, SF2->F2_PLIQUI , SF2->F2_PBRUTO})
				aadd(aEspVol,{ cEspecie, SF1->F1_ESPECI1,SF1->F1_ESPECI2,SF1->F1_VOLUME2,SF1->F1_VOLUME1,SF1->F1_PLIQUI,SF1->F1_PBRUTO}) 
...
For nX := 1 To Len(aVol)		
	...
	cString += NfeTag('<qVol>',ConvType(aVol[nX][02]))
	    ...
	    // cString += '<marca>' +aVol[03]+'</marca>'  // potencial - mil  23/04/2019
		// cString += '<nVol>'  +aVol[04]+'</nVol>'   // potencial mil 23/04/2019
