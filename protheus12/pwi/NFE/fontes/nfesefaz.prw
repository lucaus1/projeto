...
Local lIpiDev   	:= GetNewPar("MV_IPIDEV",.F.)   //Apenas para devolu��o de compra de IPI (nota de sa�da). T-S�ra gerado na tag vIPI e destacado no campo VALOR IPI do cabe�alho do danfe. F-Ser� gerado na tag vOutro e destacado nas informa��es complementares do danfe 														//e no campo OUTRAS DESPESAS ACESSORIAS
...
Local lIpiBenef   	:= GetNewPar("MV_IPIBENE",.F.) 				//Nota de sa�da de retorno com tipo = Beneficiamento. .T.- Ser� gerado na tag vOutro e destacado nas informa��es																		//complementares do danfe e no campo OUTRAS DESPESAS ACESESSORIAS. .F. - S�ra gerado na tag vIPI e destacado no campo																		//VALOR IPI do cabe�alho do danfe (procedimento padr�o)
Local lMvImpFecp    := GetNewPar("MV_IMPFECP",.F.)	                // Imprime FECP
Local lOrgaoPub	  := GetNewPar("MV_NFORGPU",.F.)				//NF-e de remessa nas opera��es de aquisi��o de �rg�o p�blico, com entrega em outro �rg�o p�blico (RICMS SP) AJUSTE SINIEF 13, DE 26 DE JULHO DE 2013 
Local oWSNfe
Local lNfCupZero	:= .F.
Local lRural		:= .F.
Local lSeekOk   	:= .F.
...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				// Tratamento para quando existir um cliente de entrega, utiliz�-lo ao inv�s do cliente de venda
			...
			Else
				...
				aAdd(aDest, "") // Posi��o 17 (referente a A1_CONTRIB, sendo passado como vazio j� que n�o existe A2_CONTRIB)
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3-4-5-6-8" .And. cCSTrib $ "00-10-20-40-60-70-90")    /*Inserido o c�digo 060 conforme consulta do chamado TIDWAX */
					...
					aadd(aProd,	{Len(aProd)+1,;
						...
						IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0))/IIF(Empty(SB5->B5_CONVDIP),(cAliasSD2)->D2_QUANT,SB5->B5_CONVDIP*(cAliasSD2)->D2_QUANT),; //RetPrvUnit(cAliasSD2,nDesconto),;
		    ...
		    //Mensgaem para ICMS Particionado - Conv�nio ICMS N� 51/00,
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					"",;
					0,;
					0,;  	// [30] Da posi��o 28 a 30 tratamento realizado apenas para documento de sa�da por este motivo campos est�o zerados e vazios.
...
If !Empty(aExp[1]) .and. lEECFAT .and. cVerAmb >= "3.10"
	If Len(aExp) > 0 .and. aNota[04] == "1" //Somente se nota de sa�da
...
cVunTrib:= ConvType(aProd[16],21,8) //ConvType(aProd[10]/aProd[12],21,8)    // potencial 

If (cVunCom <> cVunTrib) .and. lUnTribCom
	cMensFis += " "
	cMensFis += "(Valor unitario comercial: "+cVunCom+ ", "
	cMensFis += "Valor unitario tributavel: "+cVunTrib+ ") "	
	alert(" valor unit comercial "+cVunCom+"  valor unit trib "+cVunTrib)
EndIf                                                                    
If (cVunCom <> cVunTrib)
     alert(" valor unit comercial "+cVunCom+"  valor unit trib "+cVunTrib)
Endif
			...
			If nBaseIcm > 0 .and. nValICM>0
				...
				ElseIF SF4->F4_ART274 == "1"  .And. Upper(SM0->M0_ESTCOB) == "PR"  
					...
					if SF4->F4_CODIGO > "500"  /* TES de Sa�da */  
						cMensFis += " Imposto Recolhido por Substitui��o - Artigo 2., II , Anexo X , do RICMS/PR Decreto 6080/2012, onde o 'Cod.Produto:  " +ConvType(aProd[02])+" '  Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
					else //entrada
						cMensFis += " Imposto Recolhido por Substitui��o - Artigo 4., II , Anexo X , do RICMS/PR Decreto 6080/2012, onde o 'Cod.Produto:  " +ConvType(aProd[02])+" '  Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
					endif
				...
				Else
					...
					cMensFis += "Imposto Recolhido por Substitui��o - Contempla os artigos 273, 313 do RICMS. Valor da Base de ST: R$ "+str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
				EndIf
	        EndIf
...
If Len( aNfVinc ) > 0		//Nota de esp�cie NFE e CTE vinculada  
...
Funcao que retorna o pre�o unitario do produto
