...
Local cMVCOMPET:= SuperGetMV("MV_COMBPET", ,.F.)
Local cMVSUBTRIB := ""
Local cLJTPNFE	 := ""
Local cWhere	 := ""
Local cMunISS	 := ""
...
Local aMotivoCont := PARAMIXB[1,7]
...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			If ("CF" $ SF2->F2_ESPECIE) .AND. ("S" $ SF2->F2_ECF) .AND. !Empty(SF2->F2_NFCUPOM)
			...
			If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
				If lSimpNac
					...
					cMensFis += "Permite o aproveitamento do credito de ICMS no valor de R$ " +str(SF3->F3_VALICM)+ " corresponte a aliquota de "+str(SF3->F3_ALIQICM)+ " , nos termos do art. 23 da LC 123/2006. "
				EndIf
			EndIf		
			dbSelectArea("SF2")
			//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
...
//							aTail(aISSQN) := {CD2->CD2_BC,CD2->CD2_ALIQ,CD2->CD2_VLTRIB,cMunISS,cCodIss}
				...
				If SFT->(FieldPos("FT_CSTPIS")) > 0 .And. SFT->(FieldPos("FT_CSTCOF")) > 0
					...
					If MsSeek(xFilial("SFT")+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
						...
						IF Empty(aPis[Len(aPis)]) .And. SFT->FT_CSTPIS=="06" .And. SFT->FT_CSTPIS=="04" 
							aadd(aPisAlqZ,{SFT->FT_CSTPIS})
						Else
							aadd(aPisAlqZ,{})
						EndIf
						IF Empty(aCOFINS[Len(aCOFINS)]) .And. SFT->FT_CSTCOF=="06"  .And. SFT->FT_CSTCOF=="04"
				...
				Else
					...
					IF Empty(aPis[Len(aPis)]) .And. SF4->F4_CSTPIS=="06" .and. SF4->F4_CSTPIS=="04"
						aadd(aPisAlqZ,{SF4->F4_CSTPIS})
					Else
						aadd(aPisAlqZ,{})
					EndIf
					IF Empty(aCOFINS[Len(aCOFINS)]) .And. SF4->F4_CSTCOF=="06"  .And. SFT->FT_CSTCOF=="04"
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !Empty((cAliasSD1)->D1_NFORI).and.(cAliasSD1)->D1_TIPO $ "DB" .and. alltrim(SF2->F2_ESPECIE)=="CF"
				...
				If SFT->(FieldPos("FT_CSTPIS")) > 0 .And. SFT->(FieldPos("FT_CSTCOF")) > 0
					...
					If MsSeek(xFilial("SFT")+"E"+SF1->F1_SERIE+SF1->F1_DOC+SF1->F1_FORNECE+SF1->F1_LOJA+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->D1_COD)
						...
						IF Empty(aPis[Len(aPis)]) .And. SFT->FT_CSTPIS=="06" .and. SF4->F4_CSTPIS=="04"
							aadd(aPisAlqZ,{SFT->FT_CSTPIS})
						Else
							aadd(aPisAlqZ,{})
						EndIf
...
If !Empty(aNota)
	...
	cString += NfeTransp(cModFrete,aTransp,aImp,aVeiculo,aReboque,aEspVol,cVerAmb)
...
Static Function NfeDest(aDest,cVerAmb)
...
If Len(aISSQN)>0 .and. !Empty(aISSQN[01])
	...
	cString += '<Tributo>'
	cString += '<vBC>'+ConvType(aISSQN[08],15,2)+'</vBC>'
	cString += '<aliquota>'+ConvType(aISSQN[04],5,2)+'</aliquota>'
	cString += '<Valor>'+ConvType(aISSQN[05],15,4)+'</Valor>'
	cString += '</Tributo>'
	cString += '<cpl>'
	cString += '<cmunfg>'+ConvType(SM0->M0_CODMUN)+'</cmunfg>'
	cString += '<clistserv>'+aISSQN[01]+'</clistserv>'
	cString += '<cSitTrib>'+ConvType(aISSQN[09])+'</cSitTrib>'
	cString += '</cpl>'
	cString += '</imposto>'
EndIf
...
Static Function NfeTransp(cModFrete,aTransp,aImp,aVeiculo,aReboque,aVol,cVerAmb)
