...
#DEFINE MAXITEMC   036  //038                                                // Máxima de caracteres por linha de produtos/serviços
...
#DEFINE MAXVALORC  013   // 009  //Padrao                                        // Máximo de caracteres por linha de valores numéricos
...
If Type("oTransp:_Vol")<>"U"
	If ValType(oTransp:_Vol) == "A"
		...
		aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U",IIF((SM0->M0_CODIGO)=="01","MIL BRASIL",IIF((SM0->M0_CODIGO)=="03","BK ENERGIA","")),oTransp:_Vol:_Marca:TEXT) // Potencial aqui
...
For nZ := 1 To nLenDet
	...
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_TOTAL")[2])))) // POTENCIAL TamSX3("D2_PRCVEN")[2]
...
oDanfe:Say(428,302,Transform(nQuant,"@E 999,999,999.99999"),oFont08:oFont) //potencial -> aqui //oDanfe:Say(428,302,aTransp[14],oFont08:oFont)
...
For nY := 1 To nLenItens
	...
	Else
		...
		nAuxH2 := len(aAux[1][7][nY]) + (nAuxH + (aTamCol[7]) - RetTamTex(aAux[1][7][nY], oFont08:oFont, oDanfe))
		 // Alert("Valor  informar a potecial de nAuxH2:"+STR(nAuxH2)+" "+aAux[1][2][nY]) // Potencial
        nColuna := nAuxH2 
		if nColuna >= 314  // Mil 1a linha de cada produto 
		  oDanfe:Say(nLinha, 330 , aAux[1][7][nY], oFont08:oFont) // QUANT Potencial
		Elseif nColuna >= 296.5 .And. nColuna <= 313 // BK 1a linha
    		oDanfe:Say(nLinha, 318 , aAux[1][7][nY], oFont08:oFont) // QUANT Potencial
		Else      // Padrao para outras linhas 
    		oDanfe:Say(nLinha, nAuxH2, aAux[1][7][nY], oFont08:oFont) // QUANT
		Endif
		...
		if nColuna >= 314  // Mil 1a linha
		   oDanfe:Say(nLinha, 380 , aAux[1][8][nY], oFont08:oFont) // V UNITARIO Potencial
		Elseif nColuna >= 296.5 .and. nColuna <= 313 // BK 1a linha
           oDanfe:Say(nLinha, 375 , aAux[1][8][nY], oFont08:oFont) // V UNITARIO Potencial
    	Else      // Padrao para outras linhas 
        	oDanfe:Say(nLinha, nAuxH2, aAux[1][8][nY], oFont08:oFont) // V UNITARIO
    	Endif
		nAuxH += aTamCol[8]
		...
		nAuxH2 := len(aAux[1][9][nY]) + (nAuxH + (aTamCol[9]) - RetTamTex(aAux[1][9][nY], oFont08:oFont, oDanfe))
                   //408 -estava antes 02/05
		if nColuna >= 314  // Mil 1a linha de cada produto 
 			oDanfe:Say(nLinha, 408 , aAux[1][9][nY], oFont08:oFont) // V. TOTAL Potencial
		Elseif nColuna >= 296.5 .And. nColuna <= 313 // BK 1a linha
  			oDanfe:Say(nLinha, 401 , aAux[1][9][nY], oFont08:oFont) // V. TOTAL Potencial
		Else      // Padrao para outras linhas 
		...
		nAuxH2 := len(aAux[1][10][nY]) + (nAuxH + (aTamCol[10]) - RetTamTex(aAux[1][10][nY], oFont08:oFont, oDanfe))
				...
				// 443 - valor antes 02/05
		if nColuna >= 314  // Mil 1a linha de cada produto 
 			oDanfe:Say(nLinha, 443 , aAux[1][10][nY], oFont08:oFont) // BC. ICMS Potencial
		Elseif nColuna >= 296.5 .And. nColuna <= 313 // BK 1a linha
			oDanfe:Say(nLinha, 437 , aAux[1][10][nY], oFont08:oFont) // BC. ICMS Potencial
		Else      // Padrao para outras linhas 
		...
		oDanfe:Say(nLinha, 476 , aAux[1][11][nY], oFont08:oFont) // V. ICMS Potencial
		nAuxH += aTamCol[11]
...
if aAux[1][1][nItem] == "-"
 ...
 // Alert("Entrando impitem, avisar Potencial para ajustar ") 
   ...
   // 	oDanfe:Say(nLinha, 80 , aAux[1][3][nItem], oFont08:oFont) // QUANT  potencial 
	nAuxH += aTamCol[3]
	
	oDanfe:Say(nLinha, nAuxH + 2, aAux[1][4][nItem], oFont08:oFont) // V UNITARIO
	// oDanfe:Say(nLinha, 95 , aAux[1][4][nItem], oFont08:oFont) // V UNITARIO
	nAuxH += aTamCol[4]
	oDanfe:Say(nLinha, nAuxH + 2, aAux[1][5][nItem], oFont08:oFont) // V. TOTAL
	// oDanfe:Say(nLinha, 110 , aAux[1][5][nItem], oFont08:oFont) // V. TOTAL
endif
