			    ...
			    qItens2->(DbGotop())
     		    Do while !qItens2->(Eof())
			      ...
			      If Alltrim(qItens2->MAXFOR)!=""
			        @ll,002 PSay "|"+PADR(qItens2->C1_ITEM,4)+"| "+PADR(qItens2->C1_UM,2)+" |"+PADR(Transform(qItens2->C1_QUANT,"@E 99999"),5)+"|"+PADR(qItens2->C1_PRODUTO,10)+"-"+PADR(qItens2->C1_DESCRI,26)+"|"+PADR(DTOC(STOD(qItens1->MAXDATA)),10)+"| "+PADR(Transform(qItens2->MAXQTD,"@E 99999"),5)+"|"+PADR(Transform(qItens2->MAXVAL,"@E 999,999.99"),10)+"|"+ Posicione("SA2", 1, xFilial("SA2")+Alltrim(qItens2->MAXFOR) ,"PADR(A2_NOME,14)") 
			        @ll,100 Psay "|"+PADR(Transform(qItens2->C1_VLPRF1,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLNEF1,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLPRF2,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLNEF2,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLPRF3,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLNEF3,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLPRF4,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLNEF4,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLPRF5,"@E 999,999.999"),10)+"|"+PADR(Transform(qItens2->C1_VLNEF5,"@E 999,999.999"),10)+"| "+Padr(_cVenc,1)+" |"  
			       ll++
			      Else  
