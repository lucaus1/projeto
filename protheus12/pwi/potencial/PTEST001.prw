...
User Function MT240TOK()
	...
	//Variaveis para Controle de Grupos (Para acesso da Rotina)
	Private _cGrpUser := Space(25)
	Private _lLoadProg := .F.

	//Valida se o usuario pertence ao Grupo "000002" -> se sim pode acessar a rotina
	_aGrpUser := UsrRetGRP()
	For i := 1 to Len(_aGrpUser)
		if AllTrim(_aGrpUser[i]) == "000002" //Consumo
			_cGrpUser := GRPRetName(_aGrpUser[i])
			_lLoadProg := .T.
		EndIf
	Next
	
	//Verifica se o Produto permite esse controle
	if AllTrim(SB1->B1_COD) != AllTrim(GetMV("MV_YCODDIE"))
		Return .T.				
	EndIf	
	
    If _lLoadProg
		_lRet := U_PTEST001(M->D3_QUANT, M->D3_EMISSAO, M->D3_CC, M->D3_DOC)
    else
    	Alert("Usuário sem permissão para executar esse processo!!!!")
    	Return .F.
    EndIf
Return .F.
...
/*
	...
	TABELAS
		- SX5=P1  - Grupo de Veiculos
		- SN1     - Cadastro de Ativo
		- ZT6     - Histórico de Consumo de Combustivel - Tabela Customizada
		- SX2     - Dicionário de Tabelas
		- SX3     - Dicionário de Campos
		- SXB-PT1 - Consulta de Veiculo
		- SXB-CTT - Centro de Custo
		- SIX     - Indices, SN1, ZT6
	
	ROTINAS USADAS
		MT240TOK     - Ponto de Entrada - Confirmação da Requisição - Modelo I
		PTEST001.PRW - Rotina de Lancamento de Consumo    
		PTREST01.PRW - Relação de Lista de Veículos
		PTREST02.PRW - Relatório de Controle de Combustível
	
  	
  	PARAMETRO		
  		MV_YCODDIE - C - Código do Produto usado para controle de Combustivel  		
*/