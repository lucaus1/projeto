...
_cMsg9  := "Os arquivos gerados terao a extensao .DBF"
...
Else
	...
	Do while !eof()
		...
		if  ( lCont  .and. upper(substr(X2_ARQUIVO,1,_nTabela)) >= upper(_cTabela) ) .or. ;
			...
			++_nCtdite
			//                          1                         2                        3                        4                     5        6 7
			aadd(_aArqtmp,{alltrim(upper(X2_CHAVE)),alltrim(upper(X2_ARQUIVO)),alltrim(upper(X2_NOME)),alltrim(upper(X2_PATH)),strzero(_nCtdite,5),0,0})
		Endif
	...
	if _lImporta
		...
		_aFilTmp := directory(Alltrim(_cPasta)+"*.DBF")
		...
		For i:=1 to len(_aArqTmp)
			...
			If nPos<> 0
				...
				dbclosearea()
				//                               1                              2                             3                          4                            5                   6            7
				aadd(_aArqtrf,{alltrim(upper(_aArqTmp[i][1])),alltrim(upper(_aArqTmp[i][2])),alltrim(upper(_aArqTmp[i][3])),alltrim(upper(_aArqTmp[i][4])),strzero(++_nCtdite,5),_aFilPro[npos][2],_nRegCount})
			Endif
	...
	Else
		...
		For i:=1 to len(_aArqTmp)
			...
			aadd(_aArqtrf,{alltrim(upper(_aArqTmp[i][1])),alltrim(upper(_aArqTmp[i][2])),alltrim(upper(_aArqTmp[i][3])),alltrim(upper(_aArqTmp[i][4])),_aArqTmp[i][5],0,0})
		Next
	Endif
Endif
		...
		if ! select(_cArqAlias) > 0
			...
			if ! _lImporta
				...
				Else
					...
					if file(_aArqTrf[i][4]+_cArqtrf+".DBF")
...
While !Eof()
	...
	If Marked("_MARCA")
		aadd(_aArqPro,{ALLTRIM(BRW->_chave),ALLTRIM(BRW->_ARQUI),SUBSTR(BRW->_DESCR,1,20),ALLTRIM(BRW->_PATH),alltrim(transform(BRW->_TOTTAM,"@e 999,999,999")+" Kb")})
		if _lInforma
...
if _cTipPro == '1'
	...
	For i:=1 to nCont1
		...
		Else
			...
			if ! select(_cArqAlias) > 0
				If _lTOP
					...
					EndIf
				Else
					if file(_aArqPro[i][4]+_cArqtrf+".DBF")
		...
		if _lexporta
			...
			_cALiasExp := alias()
			_cArq      := Alltrim(_cPasta)+_cArqtrf + ".DBF"
			_nTamArq   := reccount()
			_ainfo     := dbstruct()
			_aArqInt   := {}
			...
			if !file(_cArq)
				...
				//_lExporta := MsgYesNo("Arquivo destino ("+_cArqtrf + ".DBF"+") ja existe. Sobrepoe ? ","Sobrepoe arquivo")
				//Endif
				//if _lExporta
				//copy to &_cArq
	...
	Else
		...
		if ! select(_cArqAlias) > 0
			If _lTOP
				...
				EndIf
			Else
				if file(_aArqPro[_nCtdPro][4]+_cArqtrf+".DBF")
...
IF  EMPTY(&_cVariavel)
	...
	Else
		...
		_cTipo := _cTipo + "Arquivos de Dados (*.DBF)  | *.DBF | "
