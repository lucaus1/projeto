...
#DEFINE MAXITEMC   040 // 18 -> aqui                                  // Máxima de caracteres por linha de produtos/serviços
...
#DEFINE MAXVALORC  012 // 008 -> aqui                                 // Máximo de caracteres por linha de valores numéricos
...
//		aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U","",NoChar(oTransp:_Vol:_Marca:TEXT,lConverte))
		aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U",IIF((SM0->M0_CODIGO)=="01","MIL BRASIL",IIF((SM0->M0_CODIGO)=="03","BK ENERGIA","")),oTransp:_Vol:_Marca:TEXT) // aqui
...
//	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],TamSX3("D2_QUANT")[2]))))
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,TamSX3("D2_QUANT")[1],3)))) //aqui
//	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],TamSX3("D2_PRCVEN")[2]))))
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],2))))   // aqui
...
nForTo		:=	Len(cStrAux)/34 //aqui
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*34)+1),34), oFont12N:oFont ) //aqui
	nLinCalc+=10
Next nX

//cStrAux		:=	AllTrim(NoChar(oEmitente:_EnderEmit:_xLgr:Text,lConverte))+", "+AllTrim(oEmitente:_EnderEmit:_Nro:Text)
cStrAux		:=	AllTrim(NoChar(oEmitente:_EnderEmit:_xLgr:Text,lConverte))  //+", "+AllTrim(oEmitente:_EnderEmit:_Nro:Text)-> Não imprimir numero 2x -> aqui
...
oDanfe:Say(428,302,Transform(nQuant,"@E 999,999,999.99999"),oFont08:oFont)//oDanfe:Say(428,302,aTransp[14],oFont08:oFont) //potencial -> aqui
oDanfe:Box(408,400,432,501)
oDanfe:Say(418,402,"PESO BRUTO",oFont08N:oFont)
//oDanfe:Say(428,402,Iif(!Empty(aTransp[15]),aTransp[15],Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][3])>0,Transform(Val(aEspVol[1][3]),"@E 999999.9999"),"")),oFont08:oFont) -> aqui
oDanfe:Say(428,402,Iif(!Empty(aTransp[15]),Transform(val(aTransp[15]),"@E 999999.9999"),Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][3])>0,Transform(Val(aEspVol[1][3]),"@E 999999.9999"),"")),oFont08:oFont)
...
//oDanfe:Say(428,502,Iif(!Empty(aTransp[16]),aTransp[16],Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][2])>0,Transform(Val(aEspVol[1][2]),"@E 999999.9999"),"")),oFont08:oFont) -> aqui
oDanfe:Say(428,502,Iif(!Empty(aTransp[16]),Transform(val(aTransp[16]),"@E 999999.9999"),Iif(Len(aEspVol)>0 .And. Val(aEspVol[1][2])>0,Transform(Val(aEspVol[1][2]),"@E 999999.9999"),"")),oFont08:oFont)
