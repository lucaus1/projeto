...
//While !eof() .and. e5_data == dData
  ...
  If e5_recpag='P' .and. &(cCond_P)
		 ...
		 If e5_banco + e5_agencia + e5_conta == aBancos[1]
		 		 ...
		 		 aDescPag[nItem] := dtoc(e5_data) + " " + left(e5_histor,40) + " / " + e5_benef + iif(!empty(e5_numcheq)," (CH." + e5_numcheq + ")",space(21)) + iif(!empty(e5_naturez)," - " + e5_naturez+" "+cDescNG,space(47))
		 		 lGrava := .t.
		 Else
		 		 aPag[nItem][1] := 0
		 Endif
  ...
  Else
     If  &(cCond_R)
		 		 ...
		 		 If e5_banco + e5_agencia + e5_conta == aBancos[1]
		 		 		 ...
		 		 		 aDescRec[nItem] := dtoc(e5_data) + " " + left(e5_histor,40) + " / " + e5_benef + iif(!empty(e5_numcheq)," (CH." + e5_numcheq + ")",space(21)) + iif(!empty(e5_naturez)," - " + e5_naturez+" "+cDescNG,space(47))
		 		 		 lGrava := .t.
		 		 Else
		 		 		 aRec[nItem][1] := 0
		 		 Endif
...
For i:=1 to nNumBco  //len(aBancos)
		 ...
		 dbSelectArea("SE8")
		 xData := dDataDe-1
		 dbSeek(xFilial('SE8')+cBanco+dtos(xData))
