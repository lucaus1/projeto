...
cQry := "SELECT E1_FILIAL, E1_NUM, E1_PREFIXO, E1_PARCELA, E1_VALOR, E1_VENCTO, E1_TIPO, E1_CLIENTE, E1_LOJA, E1_EMISSAO "
...
While ! Eof()
	...
	cDiaVenc := Substr(DTOC(STOD(E1_VENCTO)),1,2)
	cMesVenc := Upper(MesExtenso(Month( STOD(E1_VENCTO) )))
	cAnoVenc := Substr(E1_VENCTO,1,4)
	cDiaExt := alltrim( extenso( Val( Substr(DTOC(STOD(E1_VENCTO)),1,2) ) ) )
	cDiaExt := Substr(cDiaExt,1,Len(cDiaExt)-5)
	cAnoExt := alltrim( extenso( Val( Substr(E1_VENCTO,1,4) ) ) )
	...
	FOR I := 1 TO 2
		...
		oPrint:Say(nLinha+20,0800,"Emissão: "+substr(E1_EMISSAO,7,2)+"/"+substr(E1_EMISSAO,5,2)+"/"+substr(E1_EMISSAO,1,4),oFont12)
		...
		nLinha += 210
		nLinha += 45
		oPrint:Say(nLinha,0120,OemToAnsi("Aos "+cDiaExt+" dia(s) do mês de "+cMesVenc+" do ano de "+cAnoExt+" ***"),oFont11)
		nLinha += 60
		oPrint:Say(nLinha,1200,OemToAnsi("*** pagarei por esta única via de NOTA FISCAL / FATURA"),oFont11)
		nLinha += 60
		oPrint:Say(nLinha,0120,OemToAnsi("à "+alltrim(aDadosEmp[1])+" - "+aDadosEmp[6]+"ou à sua ordem a quantia de R$ "+TRANSFORM(E1_VALOR,"@E 999,999.99")),oFont11)
		...
		oPrint:Say(nLinha,1700,OemToAnsi("Data da Impressão: "+DTOC(dDataBase)),oFont11)
		nLinha += 60
		...
		oPrint:Say(nLinha+250,0120,OemToAnsi("Instruções para Pagamento: "),oFont12)
