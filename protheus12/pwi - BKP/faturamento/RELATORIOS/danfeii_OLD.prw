...
If Type("oTransp:_Transporta")<>"U"
	...
	aTransp[02] := IIF(oTransp:_ModFrete:TEXT=="0","0","1")
...
If Type("oTransp:_Vol")<>"U"
    If ValType(oTransp:_Vol) == "A"
		...
		aTransp[13] := IIf(Type("oTransp:_Vol:_Marca")=="U","MIL BRASIL",oTransp:_Vol:_Marca:TEXT)
...
For nX := 1 To nLenDet
	...
	aadd(aItens,{oDet[nX]:_Prod:_cProd:TEXT,;
					SubStr(oDet[nX]:_Prod:_xProd:TEXT,1,40),;  ///POTENCIAL 25
					...
					nQuant += nQtd //POTENCIAL
	cAux := AllTrim(SubStr(oDet[nX]:_Prod:_xProd:TEXT,40))  //POTENCIAL 26
	While !Empty(cAux)
		aadd(aItens,{"",;
					SubStr(cAux,1,40),;  //potencial 25
					"",;
					"",;
					"",;
					"",;
					...
					"",;
					""})
		cAux := SubStr(cAux,40)  //POTENCIAL 25
	EndDo
Next nX
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Quadro ISSQN                                                            ³
...
EndIf
//For nX := 1 to 20
//	aadd(amensagem,"esta mensagem é de testes para sabermos o quando ´ impresso nas mensagens da nota")
//Next nx
...
aTamanho := ImpBox(nPosV,0,0,nHPage,;
	{	{{"Base de calculo do ICMS",aTotais[01]},{"Valor do ICMS",aTotais[02]},{"Base de calculo do ICMS substituição",aTotais[03]},{"Valor do ICMS substituição",aTotais[04]},{"Valor total dos produtos",aTotais[05]}},;
		{{"Valor do Frete",aTotais[06]},{"Valor do Seguro",aTotais[07]},{"Desconto",aTotais[08]},{"\ acessórias",aTotais[09]},{"Valor do IPI",aTotais[10]},{"Valor Total da Nota",aTotais[11]}}},;
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
aTamanho := ImpBox(nPosV,0,0,nHPage,;
	{	{{"Razão Social",aTransp[01]},{"Frete por Conta","0-emitente/1-Destinatário [" + aTransp[02] + "]"},{"Codigo ANTT",aTransp[03]},{"Placa do veículo",aTransp[04]},{"UF",aTransp[05]},{"CNPJ/CPF",aTransp[06]}},;
		{{"Endereço",aTransp[07]},{"Municipio",aTransp[08]},{"UF",aTransp[09]},{"Inscrição Estadual",aTransp[10]}},;
		{{"Quantidade",aTransp[11]},{"Especie",aTransp[12]},{"Marca",aTransp[13]},{"Numeração",Str(nQuant)},{"Peso Bruto",aTransp[15]},{"Peso Liquido",aTransp[16]}}},;
...
aTamanho := ImpBox(nPosV,0,0,nHPage,;
	aAux,;
	oDanfe,3,"DADOS DO PRODUTO / SERVIÇO",{"L","L","L","L","L","L","R","R","R","R","R","R","R","R"},2)
nPosV    := aTamanho[1]+VSPACE
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Calculo do ISSQN                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
Static Function ImpBox(nPosVIni,nPosHIni,nPosVFim,nPosHFim,aImp,oDanfe,nTpFont,cTitulo,aAlign,nColAjuste)
	...
	For nY := 1 To nMaxnY
		...
		If Valtype(aImp[nX][nY]) == "A"
			...
			For nZ := 1 To nMaxnZ									
				nAuxV += Char2PixV(oDanfe,"X",aFont[nTpFont][IIf(nZ==1,1,2)])+IIF(nZ>1,SAYVSPACE*nTpFont,-1*SAYVSPACE)
				If aAlign[nY] == "R"
					oDanfe:Say(nAuxV,nAuxH-Char2Pix(oDanfe,aImp[nX][nY][nZ],aFont[nTpFont][2]),aImp[nX][nY][nZ],aFont[nTpFont][IIf(nZ==1,1,2)]:oFont)
				Else
					oDanfe:Say(nAuxV,nAuxH,aImp[nX][nY][nZ],aFont[nTpFont][IIf(nZ==1,1,2)]:oFont)
				EndIf		
			Next nZ
			nPosH1 := nPosH2
		Else
