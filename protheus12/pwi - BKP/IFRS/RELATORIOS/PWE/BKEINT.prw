...
Static Function xPrintRel()
	...
	While (!qLGRUPO->(Eof()))
	    ...
	    If Alltrim(qLGRUPO -> ZT4_TIPO) != "T" .and. Alltrim(qLGRUPO -> ZT4_TIPO) != ""
					...
					_cQuery2 += " BETWEEN '"+DTOS(MV_PAR01)+"' AND '"+DTOS(MV_PAR02)+"' AND ZT2_FILIAL='03' AND ZT2060.ZT2_CTADEB<>'' AND ZT2060.D_E_L_E_T_<>'*' GROUP BY ZT2_CTADEB) AS INTEDEB, "
			
					_cQuery2 += " (SELECT SUM(ZT2_CTAVAL) FROM ZT2060 WHERE LTRIM(ZT5060.ZT5_CONTA)=LTRIM(ZT2_CTACRE) AND ZT2_DATA "
					_cQuery2 += " BETWEEN '"+DTOS(MV_PAR01)+"' AND '"+DTOS(MV_PAR02)+"' AND ZT2_FILIAL='03' AND ZT2060.ZT2_CTACRE<>'' AND ZT2060.D_E_L_E_T_<>'*' GROUP BY ZT2_CTACRE) AS INTECRE, "
				    ...
				    While (!qTGRUPO->(Eof()))
                           ...
                           if Alltrim(qLGRUPO -> ZT4_TIPO) == "E" 				 	   
							...
							oPrint:Say(nLinha,0770, Transform((((qTGRUPO->SLDDBKE - qTGRUPO->SLDCBKE)+(qTGRUPO->AJUDBKE - qTGRUPO->AJUCBKE)  )+(qTGRUPO->INTEDEB - qTGRUPO->INTECRE))*mv_par05, "@E 999,999,999,999"), oFont07)
							...
							oPrint:Say(nLinha,1380, Transform(((qTGRUPO->SLDDBKE - qTGRUPO->SLDCBKE)+(qTGRUPO->AJUDBKE - qTGRUPO->AJUCBKE)  )*mv_par05, "@E 999,999,999,999"), oFont07)
                    ...
                    if Alltrim(qLGRUPO -> ZT4_TIPO) != "E" 				 	   
						...
						//SUBTOTAL BKE
						oPrint:Say(nLinha,1380, Transform(_SUBKERs, "@E 999,999,999,999"), oFont07)
						nLinha += 40
						oPrint:Line(nLinha,050,nLinha,1590)
	   				    nLinha += 20
	    ...
	    Elseif Alltrim(qLGRUPO -> ZT4_TIPO) =='T'
	                ...
	                _cQuery3 += " BETWEEN '"+DTOS(MV_PAR01)+"' AND '"+DTOS(MV_PAR02)+"'  AND ZT2_FILIAL='03' AND ZT2060.ZT2_CTADEB<>'' AND ZT2060.D_E_L_E_T_<>'*' GROUP BY ZT2_CTADEB) AS INTEDEB, "
	
	                _cQuery3 += " (SELECT SUM(ZT2_CTAVAL) FROM ZT2060 WHERE ZT1060.ZT1_CONTA=ZT2_CTACRE AND ZT2_DATA "
	                _cQuery3 += " BETWEEN '"+DTOS(MV_PAR01)+"' AND '"+DTOS(MV_PAR02)+"'  AND ZT2_FILIAL='03' AND ZT2060.ZT2_CTACRE<>'' AND ZT2060.D_E_L_E_T_<>'*' GROUP BY ZT2_CTACRE) AS INTECRE, "
				    ...
				    End	
						...
						//SUBTOTAL BKE
						oPrint:Say(nLinha,1380, Transform(_TOBKERs, "@E 999,999,999,999"), oFont07)
						nLinha += 40
						oPrint:Line(nLinha,050,nLinha,1590)
	   				    nLinha += 20
...
Static Function xCabec()
	...
	oPrint:Say(250,1430, "BK Energia", oFont07)
